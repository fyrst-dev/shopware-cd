...
`deploy/compose.yaml` interpolates `SHOPWARE_SHOP_ID`, `SHOPWARE_DEPLOY_ENV`, and `SHOPWARE_DATA_BASE` **directly** for bind-mount paths. Its `name:` is `${SHOPWARE_SHOP_ID}-${SHOPWARE_DEPLOY_ENV}` (e.g. `acme-live`) — the same Compose project name local `shopware-cli project dev` uses (`acme-dev` when `--env dev`). fyrst-cli passes `-p` from loaded identity. `SHOPWARE_DATA_ROOT` stays optional.
...
| `COMPOSE_PROJECT_NAME` | Not in shared `.env`. Host `.env.local`: `<shop-id>-<env>` (e.g. `acme-dev` / `acme-live`). `fyrst-cli shopware env init` **strips** create’s `sw-shop-…` line, writes this to `.env.local`, and sets gitignored `compose.override.yaml` `name:` for `shopware-cli project dev`. VPS `deploy/compose.yaml` `name:` / fyrst-cli `-p` match it. |
