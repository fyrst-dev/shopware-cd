...
- Treating fyrst-cli `-p` as the VPS Compose naming SoT, or running raw `docker compose` without `--env-file .env` then `.env.local` then `.env.prod` (if present). SoT is `deploy/compose.yaml` `name:` with those env files; `-p` is a matching pin only
