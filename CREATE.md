...
- [ ] Identity (locked hybrid): required `SHOPWARE_SHOP_ID` (stable shop slug, same on live/staging/laptop) + `SHOPWARE_DEPLOY_ENV` (`live` / `staging` / …). Optional `SHOPWARE_DATA_BASE`. `fyrst-cli shopware env init --shop-id <slug>` sets `COMPOSE_PROJECT_NAME=shopware-<slug>` (no env suffix). That env var overrides Compose `name:` so local `shopware-cli project dev` and VPS `docker compose --env-file .env …` share that name. Do not hardcode a bare Compose project name `shopware`. `SHOPWARE_DATA_ROOT` is **optional** — sync derives `SHOPWARE_DATA_ROOT=${SHOPWARE_DATA_BASE:-/var/lib/shopware/data}/${SHOPWARE_SHOP_ID}/${SHOPWARE_DEPLOY_ENV}` when unset. Compose interpolates bind-mount paths from those SoT vars directly.
- [ ] `.env` has `COMPOSE_PROJECT_NAME=shopware-<slug>` after `fyrst-cli shopware env init --shop-id <slug>` (replaces create’s `COMPOSE_PROJECT_NAME=sw-shop-…`). Flex does not set this on `composer require` — run env init.
...
- [ ] `COMPOSE_PROJECT_NAME=shopware-<slug>` — written by `fyrst-cli shopware env init --shop-id <slug>` (overrides Compose `name:`). Optional: `SHOPWARE_DATA_ROOT` — sync derives `/var/lib/shopware/data/${SHOPWARE_SHOP_ID}/${SHOPWARE_DEPLOY_ENV}` when unset. Compose interpolates bind-mount paths from the SoT vars directly.
...
- [ ] Do not comment out env init’s `COMPOSE_PROJECT_NAME=shopware-<shop-id>` (that env var is the active Compose project name; it overrides `name:`)
...
- [ ] Do not hardcode a bare Compose project name `shopware` (the pattern is `shopware-<shop-id>`, e.g. `shopware-acme`)
- [ ] Do not require `SHOPWARE_DATA_ROOT` in `.env` (optional; Compose interpolates bind-mount paths from shop id + deploy env). Do not replace env init’s `COMPOSE_PROJECT_NAME=shopware-<shop-id>` with create’s `sw-shop-…` or with `<shop-id>-<env>`
