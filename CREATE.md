...
- [ ] Do not treat fyrst-cli `-p` as the VPS Compose naming SoT, and do not run raw `docker compose` without `--env-file .env` then `.env.local` then `.env.prod` (if present). SoT is `deploy/compose.yaml` `name:` with those env files; `-p` is a matching pin only
