...
- [ ] Flex copied `.github/workflows/cd.yaml`, `.gitlab-ci.yaml`, `.dockerignore`, `.env.example`, `deploy/` (`deploy/compose.yaml`, `deploy/compose.prod.yaml`, `deploy/compose.vps.yaml`, `deploy/init-env.sh`, `deploy/sync-runtime.sh`, `deploy/sync-runtime-local.sh`)
...
- [ ] Flex may have appended a `###> fyrst/shopware-cd ###` SoT block to `.env` (empty shop id). It does **not** overwrite create’s whole `.env`. Ran `bash deploy/init-env.sh --shop-id <slug>` (see `deploy/README.md`)
...
- [ ] VPS `.env` does **not** keep create’s `COMPOSE_PROJECT_NAME=sw-shop-…` line (that **overrides** Compose `name:`). Comment it out with `bash deploy/init-env.sh --vps` (or by hand). Flex does not delete it on `composer require` (create owns local `project dev`).
...
- [ ] Runtime: `SHOPWARE_SHOP_ID` (stable shop slug) — **required** (`bash deploy/init-env.sh --shop-id <slug>`)
...
- [ ] Do not leave create’s `COMPOSE_PROJECT_NAME=sw-shop-…` in the VPS `.env` (it overrides Compose `name:`; use `bash deploy/init-env.sh --vps`)
